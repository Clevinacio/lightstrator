...
`agent` explicitly. Do not decide "it's faster if I do it myself" —
...
exception. The `writing-plans` skill ends here.

**Where the plan is.** In omp it is
`docs/superpowers/plans/YYYY-MM-DD-<feature>.md` unless the user chose
another location. Read the plan
